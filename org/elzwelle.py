...
class simpleapp_tk(tkinter.Tk):
    ...
    def initialize(self):
        ...
        self.startTimeStampsMessage = tkinter.Message(self,text="dies\nist\nein\ntest")
        self.startTimeStampsMessage.grid(row=2,column=0,sticky="EW")
        
        self.finishTimeStampsMessage = tkinter.Message(self,text="dies\nist\nein\ntest")
    ...
    def refresh(self):
        ...
        if update_time_stamp:
            ...
            self.labelVariable.set("{} ({}): {:.2f}".format(HOST_NAME, time.strftime("%H:%M:%S"), t-program_launch_time_stamp))
            update_time_stamp = False
        ...
        if time_stamps_start_dirty:
            ...
            for t in time_stamps_start:
                t2 = t - program_launch_time_stamp
                message += time.strftime("(%H:%M:%S): ",time.localtime(t)) + "{:.2f}\n".format(t2).replace(".",",")
...
if __name__ == '__main__':    
    ...
    config['serial'] = {'port':'/dev/ttyUSB1',
                        'baud':'57600',
                        'timeout':'10'}
...
#    tracemalloc.start()
    ...
    # Initialize the port    
    myPort = serial.Serial(config.get('serial', 'port'),
    ...
    def readFunc(port):
        ...
        while True:
            try:
                ...
                print("EXCEPTION readline") 
        
        print("DONE readline")
                       
    # Configure threading
    usbReader = threading.Thread(target = readFunc, args=[myPort])
    usbReader.start()
